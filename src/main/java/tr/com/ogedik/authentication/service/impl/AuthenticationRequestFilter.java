...
public class AuthenticationRequestFilter extends OncePerRequestFilter {
     ...
     * @throws ErrorException if the token cannot be parsed or the token has expired
     */
    private AuthenticationDetails createAuthenticatedUser(HttpServletRequest request) {
        ...
        try {
            ...
            String username = request.getHeader(Headers.AUTH_USER);

            return AuthenticationDetails.builder().principal(username).token(token).build();
        } catch (IllegalArgumentException e) {
            throw new ErrorException(AuthenticationErrorType.UNABLE_GET_TOKEN, "Token cannot be parsed!");
        } catch (ExpiredJwtException e) {
            throw new ErrorException(AuthenticationErrorType.TOKEN_EXPIRED, "Expired token!");
        }
    }
}
